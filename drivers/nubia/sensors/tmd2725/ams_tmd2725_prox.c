...
static int tmd2725_prox_set_enable(struct tmd2725_chip *chip, u8 enable)
{
	...
	if (enable && chip->als_enabled) {
		ams_i2c_modify(chip, chip->shadow, TMD2725_REG_ENABLE,
			TMD2725_AEN, 0);
		close_als = true;
	}
	...
	if (enable && chip->als_enabled && close_als) {
		ams_i2c_modify(chip, chip->shadow, TMD2725_REG_ENABLE,
			TMD2725_AEN, TMD2725_AEN);
	}
...
static int tmd2725_prox_thres_calibrate(struct tmd2725_chip *chip)
{
	int ret;
	u8 prox_buf[2];
	int prox_thres_near_temp = 0;
	SENSOR_LOG_INFO("enter\n");
	prox_thres_near_temp = tmd2725_mean_prox_calc(chip);
	if (prox_thres_near_temp < PS_THRESH_DATA_MIN
		|| prox_thres_near_temp > PS_THRESH_DATA_MAX) {
		SENSOR_LOG_ERROR("thres calibration failed\n");
		return -ERR_THRES_CAL;
	}
	chip->params.prox_thres_near = prox_thres_near_temp;
				...
				const char *buf, size_t size)
{
	...
	if (val) {
		...
		SENSOR_LOG_ERROR("data[0] = %d\n", data[0]);
		SENSOR_LOG_ERROR("data[1] = %d\n", data[1]);
