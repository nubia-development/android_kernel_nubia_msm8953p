...
#ifdef NUBIA_TOUCH_NVT
	ts->vreg_vddi = devm_regulator_get(dev, "vreg_vddi");
	if (IS_ERR(ts->vreg_vddi))
		dev_err(dev, "%s: Failed to get power regulator ret:%ld\n", __func__, PTR_ERR(ts->vreg_vddi));

	ts->vreg_bus = devm_regulator_get(dev, "vreg_bus");
	if (IS_ERR(ts->vreg_bus))
		dev_err(dev, "%s: Failed to get bus pullup regulator ret:%ld\n", __func__, PTR_ERR(ts->vreg_bus));

	ts->pinctrl_info.pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(ts->pinctrl_info.pinctrl)) {
		pr_err("%s: failed to get pinctrl %ld\n", __func__, PTR_ERR(ts->pinctrl_info.pinctrl));
		return;
	}
	ts->pinctrl_info.pin_active =  pinctrl_lookup_state(ts->pinctrl_info.pinctrl,
		NVT_PINCTRL_STATE_ACTIVE);
	if (IS_ERR_OR_NULL(ts->pinctrl_info.pin_active))
		pr_err("%s:can not lookup %s pinstate %ld\n", __func__,
			NVT_PINCTRL_STATE_ACTIVE, PTR_ERR(ts->pinctrl_info.pin_active));

	ts->pinctrl_info.pin_suspend =  pinctrl_lookup_state(ts->pinctrl_info.pinctrl,
		NVT_PINCTRL_STATE_SUSPEND);
	if (IS_ERR_OR_NULL(ts->pinctrl_info.pin_suspend))
		pr_err("%s:can not lookup %s pinstate %ld\n", __func__,
			NVT_PINCTRL_STATE_SUSPEND, PTR_ERR(ts->pinctrl_info.pin_suspend));
#endif
}
#else
static void nvt_parse_dt(struct device *dev)
...
#if NUBIA_TOUCH_PALM
int32_t nvt_palm_check_report(uint8_t input_id, uint8_t *data)
{
	int32_t ret = 0;
	uint8_t func_type = data[2];
	...
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PALM)) {
		ret = 1;
		NVT_LOG("Report palm detected key event.\n");
...
#if NUBIA_TOUCH_PALM
	if (ts->palm_sleep_enable  && bTouchIsAwake == 1) {
		if (nvt_palm_check_report(input_id, point_data)) {
			enable_irq(ts->client->irq);
			mutex_unlock(&ts->lock);
			return;
		}
...
static int nubia_nvt_pinctrl_select_state(struct nvt_ts_data *nvt_ts, bool active)
{
	struct nubia_nvt_pinctrl pin_info = nvt_ts->pinctrl_info;
	int ret = -EINVAL;
...
static int nubia_nvt_enable_vreg(struct device *dev, bool enable)
{
	int retval;
	...
	if (ts->vreg_bus) {
		retval = regulator_enable(ts->vreg_bus);
		if (retval < 0) {
			dev_err(dev, "%s: Failed to enable bus pullup regulator\n", __func__);
			goto exit;
		}
	}

	if (ts->vreg_vddi) {
		retval = regulator_enable(ts->vreg_vddi);
		if (retval < 0) {
			dev_err(dev, "%s: Failed to enable power regulator\n", __func__);
...
disable_pwr_reg:
	if (ts->vreg_vddi)
		regulator_disable(ts->vreg_vddi);

disable_bus_reg:
	if (ts->vreg_bus)
		regulator_disable(ts->vreg_bus);

exit:
	return retval;
}

static inline ssize_t nubia_nvt_show_error(struct device *dev,
...
static ssize_t nubia_nvt_fw_update_store(struct device *dev,
...
static inline ssize_t nubia_nvt_wakeup_gesture_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", ts->wakeup_gestrue_enable);
}

static ssize_t nubia_nvt_wakeup_gesture_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	long val = 0;
	int ret = 0;
	...
	if (0 == bTouchIsAwake) {
		NVT_ERR("device is suspend\n");
		return count;
	}

	ts->wakeup_gestrue_enable = val;

#ifdef CONFIG_NUBIA_LCD_KEEP_POWER_ON
	nubia_wakeup_gesture = ts->wakeup_gestrue_enable;
#endif

	return count;
}

static inline ssize_t nubia_nvt_palm_sleep_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", ts->wakeup_gestrue_enable);
}

static ssize_t nubia_nvt_palm_sleep_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	long val = 0;
	int ret = 0;
...
static struct device_attribute nubia_attrs[] = {
	__ATTR(start_update, (S_IRUGO | S_IWUSR | S_IWGRP),
		nubia_nvt_show_error,
		nubia_nvt_fw_update_store),

	__ATTR(wake_gesture, (S_IRUGO | S_IWUSR | S_IWGRP),
		nubia_nvt_wakeup_gesture_show,
		nubia_nvt_wakeup_gesture_store),

	__ATTR(palm_sleep, (S_IRUGO | S_IWUSR | S_IWGRP),
		nubia_nvt_palm_sleep_show,
		nubia_nvt_palm_sleep_store),

};

static int nubia_create_sysfs(void)
{
	int nubia_attr_count, ret = 0;

	for (nubia_attr_count = 0; nubia_attr_count < ARRAY_SIZE(nubia_attrs); nubia_attr_count++) {
		ret = sysfs_create_file( &ts->client->dev.kobj, &nubia_attrs[nubia_attr_count].attr);
		if (ret < 0) {
			dev_err(&ts->client->dev, "%s: Failed to create sysfs attributes ret:%d\n", __func__, ret);
...
nubia_err_sysfs:
	for (nubia_attr_count--; nubia_attr_count >= 0; nubia_attr_count--)
		sysfs_remove_file(&ts->client->dev.kobj, &nubia_attrs[nubia_attr_count].attr);
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*add by wuyujing, NX612 noveteck touchscreen adapt for boe lcd*/
	if (nubia_panel_id != 3) {
		pr_err("%s not fount noveteck touchscreen!\n", __func__);
		return -ENODEV;
	}
...
#ifdef NUBIA_TOUCH_NVT
	nubia_nvt_enable_vreg(&ts->client->dev, true);
	nubia_nvt_pinctrl_select_state(ts, true);
#endif
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if (ts->wakeup_gestrue_enable) {
		...
		CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 2);
#ifdef NUBIA_TOUCH_NVT
		nubia_nvt_pinctrl_select_state(ts, false);
#endif
	}
	/* release all touches */
#if MT_PROTOCOL_B
