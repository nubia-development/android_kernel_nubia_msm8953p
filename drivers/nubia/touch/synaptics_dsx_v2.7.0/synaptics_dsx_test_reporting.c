...
exit:
	if (do_reset) {
		dev_info(dev, "func %s reset_device start\n", buf);
		rmi4_data->reset_device(rmi4_data, false);
		dev_info(dev, "func %s reset_device end\n", buf);
	}
	return retval;
}
...
static ssize_t test_sysfs_differ_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[] = {"2"};
...
#if 0
	dev_info(dev, "test_sysfs_tddi_noise_store\n");
	test_sysfs_tddi_noise_store(dev, attr, type, sizeof(type));
	test_sysfs_tddi_noise_show(dev, attr, buf);
#endif
