...
static ssize_t nubia_fwu_sys_ic_ver_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
#ifdef NUBIA_TOUCH_SYNAPTICS
static int nubia_fwu_sys_ic_get_firmware_id(unsigned int *firmware_id)
{
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
static int nubia_fwu_get_device_config_id(char config_id[32])
{
	int retval;
	unsigned char config_id_size;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
		...
		struct device_attribute *attr, char *buf)
{
	...
	nubia_fwu_sys_ic_get_firmware_id(&firmware_id);

	nubia_fwu_get_device_config_id(config_id);

	return snprintf(buf, PAGE_SIZE, "firmware_id:%u\nconfig_id:%02x%02x%02x%02x\n",
		firmware_id, config_id[0], config_id[1], config_id[2], config_id[3]);
}
static inline ssize_t nubia_fwu_sys_update_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
